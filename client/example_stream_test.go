...
// Example_streamBodyReader sets Request.BodyMode = BodyStream so Do returns as
// soon as the response HEADERS arrive, exposing the body as an io.ReadCloser on
...
func Example_streamBodyReader() {
	...
	req.BodyMode = client.BodyStream // body arrives lazily via Response.BodyReader

	var resp client.Response
	if err := c.Do(ctx, req, &resp); err != nil {
		log.Fatal(err)
	}
	// BodyReader is non-nil only because BodyMode was BodyStream. Always Close it.
