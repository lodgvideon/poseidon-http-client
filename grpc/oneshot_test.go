...
func TestFusedOpen_LeavesTheStreamSendComplete(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("openStream(fuse): %v", err)
	}
	defer func() { _ = s.Close() }()

	if !s.sentEnd {
		t.Error("a fused open left sentEnd false — the request was half-closed on the wire, " +
			"so a later Send would put DATA after END_STREAM")
	}
	// The flag's whole purpose: further sends are refused.
	if err := s.Send(context.Background(), []byte("more")); !errors.Is(err, ErrSendClosed) {
		t.Errorf("Send after a fused open = %v, want ErrSendClosed", err)
	}
	if err := s.SendLast(context.Background(), []byte("more")); !errors.Is(err, ErrSendClosed) {
		t.Errorf("SendLast after a fused open = %v, want ErrSendClosed", err)
	}
}
...
func TestUnfusedOpen_LeavesTheStreamOpen(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("openStream(no fuse): %v", err)
	}
	defer func() { _ = s.Close() }()

	if s.sentEnd {
		t.Fatal("opening a streaming call half-closed the request side")
	}
	if err := s.Send(context.Background(), []byte("first")); err != nil {
		t.Errorf("Send on a freshly opened streaming call = %v, want nil", err)
	}
}
...
func TestFusedOpen_EmptyRequestIsStillAMessage(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Invoke with an empty request: %v", err)
	}
	if len(got) != 0 {
		t.Errorf("echo of an empty request returned %d bytes, want 0", len(got))
	}
}
