...
func TestInvoker_AcceptsASubstitute(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("callThrough: %v", err)
	}
	if string(got) != "world" {
		t.Errorf("reply = %q, want \"world\"", got)
	}
	if f.gotMethod != "/helloworld.Greeter/SayHello" {
		t.Errorf("method = %q", f.gotMethod)
	}
	if string(f.gotReq) != "hello" {
		t.Errorf("request = %q", f.gotReq)
	}
	...
	if f.gotOpts != 1 {
		t.Errorf("substitute received %d options, want 1", f.gotOpts)
	}
}
...
func TestInvoker_ErrorsPropagate(t *testing.T) {
	...
	if _, err := callThrough(context.Background(), f, nil); !errors.Is(err, sentinel) {
		t.Fatalf("error = %v, want the substitute's own", err)
	}
}
...
func TestInvoker_RealConnSatisfiesItAndWorks(t *testing.T) {
	...
	got, err := inv.Invoke(context.Background(), "/bench.Svc/Echo", []byte("hello"), nil)
	if err != nil {
		t.Fatalf("Invoke through Invoker: %v", err)
	}
	if string(got) != "hello" {
		t.Errorf("echo = %q, want \"hello\"", got)
	}

	s, err := inv.NewStream(context.Background(), "/bench.Svc/Echo", nil)
	if err != nil {
		t.Fatalf("NewStream through Invoker: %v", err)
	}
...
func TestInvoker_WrapsForMiddleware(t *testing.T) {
	...
	if _, err := callThrough(context.Background(), mw, []byte("q"), caller); err != nil {
		t.Fatalf("through middleware: %v", err)
	}
	if f.gotOpts != 2 {
		t.Errorf("connection saw %d options, want 2 (caller's plus the wrapper's)", f.gotOpts)
	}
}
