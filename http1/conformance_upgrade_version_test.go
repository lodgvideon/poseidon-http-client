...
func TestConformance_RFC9110_Sec7_8_UnsolicitedUpgradeRejected(t *testing.T) {
	...
	require.ErrorIsf(t, err, http1.ErrUnsolicitedUpgrade,
		"ReadResponse = (%d, %v), want ErrUnsolicitedUpgrade — the fabricated "+
			"200 that follows the 101 must never be returned as this request's response", status, err)
	assert.False(t, ex.KeepAlive(),
		"KeepAlive() = true after a 101, want false — the peer considers the "+
...
func TestConformance_RFC9112_Sec2_3_VersionMustBeTwoDigits(t *testing.T) {
	for _, v := range []string{"HTTP/1.00", "HTTP/1.", "HTTP/1", "HTTP/11", "HTTP/2.0", "HTTP/1.0.1"} {
		t.Run(v, func(t *testing.T) {
			...
			require.ErrorIsf(t, err, http1.ErrInvalidStatusLine,
				"version %q: err = %v, want ErrInvalidStatusLine", v, err)
			assert.False(t, ex.KeepAlive(), "KeepAlive() = true after a malformed status line, want false")
		})
	}
	...
	for _, tc := range []struct {
		...
		t.Run("accepted "+tc.version, func(t *testing.T) {
			...
			_, _, err := ex.ReadResponse(context.Background())
			require.NoErrorf(t, err, "ReadResponse(%s): %v", tc.version, err)

			_, done, err := ex.ReadBodyChunk(make([]byte, 8))

			require.Truef(t, done && err == nil, "body should end immediately: done=%v err=%v", done, err)
			got := ex.KeepAlive()
			assert.Equalf(t, tc.keepAlive, got, "%s: KeepAlive() = %v, want %v", tc.version, got, tc.keepAlive)
		})
	}
}
...
func TestConformance_RFC9112_Sec6_1_NoChunkedToObservedHttp10Peer(t *testing.T) {
	...
	require.NoError(t, first.WriteRequest(context.Background(), head, true), "WriteRequest")
	_, _, err = first.ReadResponse(context.Background())
	require.NoError(t, err, "ReadResponse")
	_, done, err := first.ReadBodyChunk(make([]byte, 8))
	require.Truef(t, done && err == nil, "body should end immediately: done=%v err=%v", done, err)
	require.True(t, first.KeepAlive(),
		"the 1.0 + keep-alive response should be poolable; the test needs that shape")
	...
	require.ErrorIsf(t, err, http1.ErrInvalidRequest,
		"WriteRequest(chunked to an observed HTTP/1.0 peer) = %v, want ErrInvalidRequest", err)
	// Over-rejection guard: the same request with a Content-Length is fine.
	third := c.NewExchange()
	err = third.WriteRequest(context.Background(), []header.Field{
		{Name: []byte(":method"), Value: []byte("POST")},
		{Name: []byte(":path"), Value: []byte("/")},
		{Name: []byte(":authority"), Value: []byte("example.com")},
		{Name: []byte("content-length"), Value: []byte("2")},
	}, false)
	require.NoErrorf(t, err, "WriteRequest with Content-Length to a 1.0 peer = %v, want nil", err)
}
