...
**Managed transports offer the resolved Address, if your dialer wants it.** A dialer may additionally implement `client.AddressDialer` (`DialAddress(ctx, addr client.Address) (net.Conn, error)`) to receive the `Address` a `Selector` picked — host, port, and any `Attributes` the `Resolver` attached (availability zone, weight class, …) — instead of a flattened `"host:port"` string. `TransportH1Managed` and the HTTP/2 `TransportManaged` check for it on every dial a sub-pool makes for a resolved address, preferring `DialAddress` over `Dial` whenever the configured dialer implements it; `Dial` is still called when it does not, so existing dialers keep working unchanged. `Attributes` is the same map instance the `Resolver`/`Selector` hold — treat it as read-only.
