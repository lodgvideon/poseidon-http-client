...
- `Priority *frame.Priority`, `Protocol string` (extended CONNECT, RFC 8441;
  requires `Method == "CONNECT"`), `Idempotent *bool`, `Trailers` /
...
To override the method-based default, set `Request.Idempotent`, a `*bool`. `nil` (the zero value) means "classify by method"; a non-nil pointer forces the answer. This lets you opt a `POST` into retry (when you know it is safe, e.g. an idempotency-key-protected endpoint) or opt a `PUT` out.
...
Note that `Idempotent` is only one of three gates the `Retryer` checks (see "What makes a request retryable" below). A request with a streaming `BodyReader` is never retried regardless of idempotency, because the reader cannot be rewound.
...
1. `MaxAttempts > 1`, and
2. the request is idempotent (`isIdempotent`, including the `Idempotent` override), and
