...
- **INV-6 (ACK cadence).** The reader holds `c.mu` across the whole `recvDatagram` + `drainBuffered`(≤512) + terminal `flush`, so no `Do` can inject a send mid-burst and the "one ACK per receive batch" property is exact. `Do`-side sends (`flushControl`, request STREAM frames) never touch `acks[sp]`. **Deliberate, legal wire change:** credit grants no longer coalesce into the ACK packet (the reader's `flush` used to merge them) — slightly more, smaller ack-eliciting packets and more peer ACK traffic. Note it in the 2c PR description so interop packet-count diffs are not mistaken for bugs.
