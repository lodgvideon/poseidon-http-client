...
| §9.2.2 | Conformance | TestConformance_RFC9113_Sec9_2_2_TLS12CipherSuitesProhibited, TestConformance_RFC9113_Sec9_2_2_ExplicitCipherSuitesRespected (conn/) — the h2-only TLSDialer pins its TLS 1.2 offer to the six forward-secret AEAD suites, offering none of RFC 9113 Appendix A's prohibited suites ("A deployment of HTTP/2 over TLS 1.2 SHOULD NOT use any of the prohibited cipher suites listed in Appendix A") while keeping the mandated TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256; a caller's explicit CipherSuites is respected, and FlexDialer (which also offers http/1.1) is deliberately left unconstrained so the §9.2.2 advertise-for-fallback note still holds. TestH2TLS12CipherSuites_FreshSlicePerCall guards against a shared backing array |
...
| §8.2.1 | Conformance | TestConformance_RFC9113_Sec8_2_1_ResponseFieldValueEdgeWhitespace_Malformed (conn/) — a RECEIVED response (or pushed-request) field value that starts or ends with SP/HTAB is malformed ("A field value MUST NOT start or end with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09)"), yielding a stream error PROTOCOL_ERROR (§8.1.2.6, connection survives); the 7540 validFieldValue only caught CR/LF/NUL. TestConformance_RFC9113_Sec8_2_1_InternalWhitespaceValueAccepted is the over-rejection guard — internal SP/HTAB stays legal. Pairs with the §8.2.1 send-side row above (client refuses to emit such a value) |
| §6.8 | Conformance | TestConformance_RFC9113_Sec6_8_GoAwaySendPeerScopedAndMonotonic, TestConformance_RFC9113_Sec6_8_GoAwaySendPushScoped, TestConformance_RFC9113_Sec6_8_GoAwayReceivedClampsRaisedLastID (conn/) — a GOAWAY the client SENDS advertises the highest peer-initiated (even, server-pushed) stream it acted on (`lastPromisedID`, 0 when push is unused), never the client's own odd request ids, and a later GOAWAY never advertises a larger id than an earlier one ("Endpoints MUST NOT increase the value they send in the last stream identifier"); on the RECEIVE side a second GOAWAY that raises the last-stream-id is clamped to the prior value while one that lowers it is honored. goAwayLastStreamIDToSend clamps against goAwaySentLast; onGoAwayReceived clamps against goAwayLastStreamID |
| §6.1 / §6.3 / §6.4 / §6.7 / §6.8 | Conformance | TestConformance_RFC9113_Sec6_1_DataFrameOnStreamZero_FramerRejects, TestConformance_RFC9113_Sec6_3_PriorityFrameOnStreamZero_FramerRejects, TestConformance_RFC9113_Sec6_4_RSTStreamFrameOnStreamZero_FramerRejects, TestConformance_RFC9113_Sec6_7_PingFrameOnNonzeroStream_FramerRejects, TestConformance_RFC9113_Sec6_8_GoAwayFrameOnNonzeroStream_FramerRejects (frame/) — receiver-side stream-identifier validation: DATA/PRIORITY/RST_STREAM on stream 0, and PING/GOAWAY on a nonzero stream, are each rejected by the Framer on read with the connection-fatal ErrInvalidStreamID sentinel ("An endpoint MUST treat a GOAWAY frame with a stream identifier other than 0x00 as a connection error (Section 5.4.1) of type PROTOCOL_ERROR", plus the parallel per-frame MUSTs in §6.1/§6.3/§6.4/§6.7). The stream-id check precedes the length check in each dispatch, so a well-formed payload still trips it (and removing the guard flips the test); §5.4.1's typed-GOAWAY-with-code is only SHOULD, so the tests assert the sentinel, not a code. Closes the sibling-divergence gap where only HEADERS/CONTINUATION had a receiver-side stream-0 test |
