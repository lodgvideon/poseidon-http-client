...
Status: **shipped**, as `conn.SendBatch` + `ConnOptions.WriteBufferSize` +
`examples/h2gen`. R0 ran first and **refuted two of the three proposed phases**;
R1 then shipped and the API it shipped as is *not* the one §5.1 sketched.

Read in this order: **§7a** (what R0 refuted), **§7b** (what shipped and why the
API changed shape), then the rest as background. §5 is kept as written for the
record — §7b says which parts of it survived.
...
Three things landed, plus a bug found on the way.

### The API is a batched send, not a frame port

§5.1 sketched `Conn.WriteFrames(ctx, []FrameChunks)` — the caller owns the wire
bytes, the connection charges flow control and writes them. What shipped is:

```go
func (c *Conn) SendBatch(ctx context.Context, batch []BatchEntry) error

type BatchEntry struct {
    Stream    StreamRef      // the lifetime, never a *Stream
    Fields     []header.Field // nil to send body only
    Body       []byte
    BodyV      [][]byte       // the vectored form; wins when non-nil
    EndStream  bool
    Err        error          // out: this entry's own outcome
}
```

The name changed because the shape did, and `WriteFrames` would now claim
something the call does not do. Four reasons, in descending order of how much
they cost to ignore:

1. **Caller-encoded HPACK was the point, and R0 refuted it.** §7a: +77 bytes per
   request to save 1.4% of CPU. Take that away and "the request owns its wire
   bytes" buys only the body — which `SendDataV` has delivered since #347, with
   no copy.
2. **A caller-chosen stream id cannot be made safe.** `assignStreamIDLocked`
   advances `c.nextID`; an id the caller picked does not, so a later `NewStream`
   reissues it and the peer answers §5.1.1 with PROTOCOL_ERROR.
3. **A caller-chosen frame *type* escapes the connection's bookkeeping.** A raw
   RST_STREAM leaks a MAX_CONCURRENT_STREAMS slot (`releaseInflight` never
   runs); a raw PING collides in `pingWaiters` and steals a real `Ping`'s ACK; a
   raw WINDOW_UPDATE desyncs `connRecvWindow` and the BDP tuner; a raw GOAWAY
   breaks §6.8's monotonic clamp on `goAwaySentLast`.
4. **Nothing measured wanted it.** The entire measured win is coalescing, and
   coalescing needs a submission point, not caller-built bytes.

So the batch is a list of ordinary sends and the connection builds the frames.
Deliberately non-conformant emission stays R5, unbuilt and separate.

### What the design's hazards turned into

- **No new per-stream field**, so `recycleStream` needs no new reset and the
  connection-reuse hazard does not arise. This was a design constraint, not an
  accident: `resetForPoolLocked` enumerates every field by hand and
  `TestRecycleStream_ReusesTheEventChannel` gates it at zero allocations.
- **`StreamRef`, never `*Stream`** — §6 hazard 2 / #370, satisfied by
  construction.
- **Not routed through `writeBatcher`.** `SendBatch` ends in `flushNow`, not
  `commitFrame`: it batches frames the caller has already handed over and waits
  for nothing, where group commit batches by making a writer *wait*. It still
  does the convoy bookkeeping, so a group-commit writer parked behind a batch is
  released by the flush that carried its bytes. Pinned by
  `TestSendBatch_ReleasesDeferringWriter`, whose negative control is the
  pre-existing `TestFlushNow_PlainFlushLeavesWriterParked`.
- **`net.Buffers` was not built.** §5.1 proposed a plaintext `writev` fast path.
  It is not needed for the stated goal and would work against it: below the write
  buffer, one flush is already one write, so `net.Buffers` would only save a
  memcpy; above it, the vector would have to be reassembled anyway. Worse, it
  degrades to one `Write` — and one TLS record — per buffer through anything that
  is not a `*net.TCPConn`, which includes `*tls.Conn` and any wrapping dialer,
  so the fast path would *raise* writes/req exactly where it could not be
  feature-detected. `ConnOptions.WriteBufferSize` reaches the same place with
  none of that.
- **Flow control per entry, taken without waiting.** Blocking under the write
  lock is the documented deadlock; an entry that does not fit reports
  `ErrNoCredit`. What that leaves behind is deterministic and documented: the
  HEADERS went out (they are not flow-controlled), the body did not, END_STREAM
  did not. Emitting the HEADERS anyway is what keeps an id from being *burned* —
  §5.1.1 forbids using a skipped id later, so bailing before the write would
  strand it permanently.

### Measured

`conn/bench_loadgen_test.go`, h2c, 64-byte request and response, this machine
(the R0 numbers in §7a are from a faster one — read the ratios, not the ns):

| bench | writes/req | wirebytes/req | ns/op |
|---|---|---|---|
| `Unary_C64` (R0 baseline) | 2.002 | 89.02 | 27075 |
| `Fused_C64` (item 1) | 1.002 | 89.02 | 16553 |
| `Batch8_C8` | 0.127 | 89.02 | 6767 |
| `Batch32_C8` | **0.033** | 89.02 | 4036 |

`wirebytes/req` is identical to two decimal places across all four, which is the
claim stated as an invariant: batching changes how many times the bytes reach the
socket and nothing else.

End to end through `examples/h2gen` against an h2c peer, 3s, one connection:

| run | writes/req | rps | p50 |
|---|---|---|---|
| `-batch 1` (control) | 1.002 | 62 174 | 962 µs |
| `-batch 32` | 0.036 | 108 389 | **555 µs** |
| `-batch 128 -writebuf 262144` | 0.014 | 154 838 | 1.57 ms |

Note the p50 at `-batch 32`: latency *improved* while throughput rose 74%. That
is the difference from #360 made visible — #360 measured group commit's waiting
mechanism, and `SendBatch` does not wait. At `-batch 128` the queueing delay does
start to show, which is the caller's knob to set.

### Item 1 was already done

Issue #438 item 1 (one-shot HEADERS+DATA+END_STREAM, "2.002 → ~1.0 writes/req,
no new API") shipped earlier as `StreamRef.SendHeadersAndData` /
`SendHeadersAndDataV` over `writeHeadersAndDataVec`. Measured at **1.002
writes/req**, so the target is met. `BenchmarkLoadGen_Fused_*` pins it, since
nothing did before — the R0 harness used the split send throughout.

### Bug found on the way

`writeGoAwayBestEffort` armed a 200 ms write deadline on the transport and never
cleared it, where its sibling `writeRSTStreamBestEffort` arms and clears the same
kind. Under `Close` that is invisible — the transport dies immediately after.
Under `Shutdown` it is not: the whole contract of `Shutdown(gracefulTimeout)` is
that the connection stays alive so in-flight streams can finish, and every one of
their writes inherited a deadline 200 ms after the GOAWAY. Any drain longer than
200 ms failed the very sends it existed to permit, with the socket's own
`i/o timeout`. Fixed, with `TestShutdown_DoesNotStrandAWriteDeadline`, which
fails on the pre-fix code.

### Still open

- **R2 / R3 stay shelved** with their numbers in §7a. Do not re-propose without
  new evidence.
- **`Conn.Stats().BytesSent` and `.BytesReceived` are permanently 0** —
  `atomicBytesSent` and `atomicBytesReceived` are declared and read by `Stats()`
  and written nowhere in the repo. Unrelated to this work and left alone, but it
  is why `examples/h2gen` counts writes with its own wrapping dialer rather than
