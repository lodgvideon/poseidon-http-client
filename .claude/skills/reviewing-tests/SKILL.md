...
**Mutate the code under test and confirm the test goes red.** Not a thought
experiment: change the covered line, run, observe. Twice, and print how many tests
actually ran — `0 tests ran` and `caught` look identical in a summary.

Checks, in descending order of how often they catch something here:

- **The mutation must land on the site you named.** An unanchored replacement takes
  the file's first match instead. Assert the edit applied, and at which line.
- **A build failure is a VOID verdict, never a catch.** A mutation that does not
  compile proves nothing. If the harness prints `[build failed]`, the row is void —
  re-do the mutation in a form that compiles.
- **Commit before mutating**, and restore afterwards. `git checkout <file>` destroys
