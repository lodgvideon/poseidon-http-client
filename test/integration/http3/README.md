...
**matrix of three real, independent HTTP/3 server stacks** over real UDP:

- [Caddy](https://caddyserver.com/) — `quic-go` (Go),
- [nginx](https://nginx.org/) — its own C QUIC implementation, and
- [aioquic](https://github.com/aiortc/aioquic) — pure Python (QPACK via ls-qpack).

Each scenario runs against every server as a subtest (`TestInterop_GET/caddy`,
`TestInterop_GET/nginx`, `TestInterop_GET/aioquic`, …), so cross-implementation
interop bugs — three independent QUIC transports and H3 layers: ACK strategy,
MAX_STREAMS replenishment, flow-control cadence, connection-ID rotation,
key-update timing — surface, not just one vendor's behaviour.
...
via `H3_INTEROP_ADDRS` (`caddy=h3caddy:443,nginx=h3nginx:443,aioquic=h3aioquic:443`);
the single-server `H3_INTEROP_ADDR` is still honoured (the loss harness uses it).
nginx is gated on container start rather than a healthcheck — its image ships no
`curl`/`wget` — so the runner retries the dial until nginx is serving; aioquic
pip-installs at startup and signals readiness with a `/tmp/ready` file the compose
healthcheck polls, so the runner waits for it without a raised dial timeout.

## What it exercises

The full stack end to end against three strict, independent implementations
(Caddy on `quic-go`, nginx on its own C stack, aioquic on pure Python): the QUIC
v1 + TLS 1.3 handshake, transport parameters, 1-RTT keys, the HTTP/3 control
stream + SETTINGS, a QPACK-encoded request, and the decoded response HEADERS +
DATA.
