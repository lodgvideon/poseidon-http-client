...
//
// pooled selects the reader lifecycle: true is what roundTrip does (a recycled
// backing array, acquire/release around the exchange), false is a cold reader
// growing from nothing on every response. The difference between the two is what
// pooling buys, and it is most of the number this file exists to report.
func feedResponse(b *testing.B, wire []byte, wantBody, burst int, pooled bool) {
	b.Helper()
	c := &Client{}
	for i := 0; i < b.N; i++ {
		var fr FrameReader
...
// body travels FrameReader.buf -> respBuilder.body. That is the http3 half of the
// copies issue #342 is about; the other, in quic recvStream, is below this layer.
//
// reader=pooled is production (roundTrip). reader=cold isolates the copy the body
// now pays: with no reuse to amortize it, it is a straight regression against
// adopting the reader's bytes — which is precisely why the copy could not ship
// without the pool, and why the pool could not ship without the copy.
	...
	for _, size := range []int{256, 4 << 10, 16 << 10, 64 << 10} {
		for _, burst := range []int{1200, 1 << 30} { // one QUIC packet, then all at once
			for _, pooled := range []bool{true, false} {
				name := fmt.Sprintf("body=%d/burst=%s/reader=%s", size, burstName(burst), readerName(pooled))
				b.Run(name, func(b *testing.B) {
					wire := AppendData(nil, make([]byte, size))
					b.ReportAllocs()
					b.ResetTimer()
					feedResponse(b, wire, size, burst, pooled)
				})
			}
		}
	}
...
// body accumulates across payloads instead of being built from one.
func BenchmarkRecvPath_BodyAcrossFrames(b *testing.B) {
	...
	for _, frames := range []int{2, 8} {
		b.Run(fmt.Sprintf("frames=%d", frames), func(b *testing.B) {
			...
			b.ReportAllocs()
			b.ResetTimer()
			feedResponse(b, wire, frames*chunk, 1200, true)
		})
	}
}

func readerName(pooled bool) string {
	if pooled {
		return "pooled"
	}
	return "cold"
