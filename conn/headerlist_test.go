...
func TestAdvertisedSettings_Defaulted_BoundsHeaderListSize(t *testing.T) {
	...
	if s.MaxHeaderListSize == 0 {
		t.Fatal("MaxHeaderListSize defaulted to 0 — decompressed-list DoS gate is off")
	}
	if s.MaxHeaderListSize != defaultMaxHeaderListSize {
		t.Fatalf("MaxHeaderListSize = %d, want %d", s.MaxHeaderListSize, defaultMaxHeaderListSize)
	}
}
...
func TestAdvertisedSettings_Defaulted_PreservesHeaderListSize(t *testing.T) {
	...
	if s.MaxHeaderListSize != 1<<20 {
		t.Fatalf("caller value lost: got %d, want %d", s.MaxHeaderListSize, 1<<20)
	}
}
...
func TestEncodeAdvertised_AnnouncesHeaderListSize(t *testing.T) {
	...
	if !ok {
		t.Fatal("SETTINGS_MAX_HEADER_LIST_SIZE not advertised")
	}
	if v != defaultMaxHeaderListSize {
		t.Fatalf("advertised = %d, want %d", v, defaultMaxHeaderListSize)
	}
}
...
func TestConformance_RFC7540_Sec10_5_1_HeaderListSizeCap_EnhanceYourCalm(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected ConnError for over-limit decoded header list")
	}
	ce, ok := err.(*ConnError)
	if !ok {
		t.Fatalf("err type = %T, want *ConnError", err)
	}
	if ce.Code != frame.ErrCodeEnhanceYourCalm {
		t.Fatalf("code = %v, want ENHANCE_YOUR_CALM (RFC 7540 §10.5.1)", ce.Code)
	}
}
...
func TestHeaderListSizeCap_WithinLimit_Succeeds(t *testing.T) {
	...
	if err := h.OnHeaders(fh, frame.HeaderBlock(block), nil, 0); err != nil {
		t.Fatalf("OnHeaders under cap: %v", err)
	}
	select {
	case e := <-s.events:
		if e.Type != EventHeaders {
