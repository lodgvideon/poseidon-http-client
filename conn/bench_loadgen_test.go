...
// --- R1: the two send paths that reduce writes/req.
//
// The macro benchmarks above use SendHeaders + SendData, which is two writes per
// request and is what R0 measured. These are the paths that fix that, and the
// three-way comparison is the whole argument:
//
//	Unary_C64          2.002 writes/req   HEADERS and DATA sent separately
//	Fused_C64          1.002              one request, one write
//	Batch32_C64        ~0.04              32 requests, one write
//
// Neither number is a latency claim — a real socket is in the loop — but
// writes/req is deterministic, and on h2c it is the syscall count.

// benchLoadGenFused is benchLoadGen using the fused one-shot send, which puts a
// request's HEADERS and its whole body in a single write.
func benchLoadGenFused(b *testing.B, workers, reqSize, respSize int) {
	b.Helper()
	p := newLoadGenPeer(b, respSize)
	wc := &lgWriteCounter{}
	c := dialLoadGenPeerOpts(b, p, wc, false)
	ctx := context.Background()
	hdrs := lgRequestFields("bench.local")
	body := make([]byte, reqSize)
	...
	one := func() error {
		...
		if err := s.SendHeadersAndData(ctx, hdrs, body, true); err != nil {
			return err
		}
		return drainLoadGenStream(ctx, s)
	}
	runLoadGen(b, wc, workers, one)
}

// benchLoadGenBatch coalesces batchSize requests into one SendBatch call. Each
// worker owns its own batch, so the benchmark measures the batching rather than
// contention on a shared queue — a real generator's queue is its own design
// problem and belongs in examples/h2gen, not here.
func benchLoadGenBatch(b *testing.B, workers, batchSize, reqSize, respSize int) {
	b.Helper()
	p := newLoadGenPeer(b, respSize)
	wc := &lgWriteCounter{}
	c := dialLoadGenPeerOpts(b, p, wc, false)
	...
	// Per-worker state, allocated once: this is how a generator uses the API,
	// and allocating a batch per call would charge the benchmark for the
	// caller's mistake rather than for the path's cost.
	pool := make(chan []BatchEntry, workers)
	for i := 0; i < workers; i++ {
		pool <- make([]BatchEntry, batchSize)
	}

	one := func() error {
		batch := <-pool
		defer func() { pool <- batch }()
		for i := range batch {
			s, err := c.NewStream(ctx)
			if err != nil {
				return err
			}
			batch[i] = BatchEntry{Stream: s, Fields: hdrs, Body: body, EndStream: true}
		}
		if err := c.SendBatch(ctx, batch); err != nil {
			return err
		}
		for i := range batch {
			// ErrNoCredit is expected at this concurrency and is not a failure:
			// 64 workers x 32 entries is more in-flight body than the 65535-byte
			// connection window allows, and SendBatch refuses rather than
			// blocking under the write lock. The generator finishes those the
			// blocking way, which is the documented contract — and the writes it
			// costs are counted, so writes/req stays honest about it.
			switch {
			case batch[i].Err == ErrNoCredit:
				if err := batch[i].Stream.SendData(ctx, body, true); err != nil {
					return err
				}
			case batch[i].Err != nil:
				return batch[i].Err
			}
			if err := drainLoadGenStream(ctx, batch[i].Stream); err != nil {
				return err
			}
		}
		return nil
	}
	runLoadGenN(b, wc, workers, batchSize, one)
}

// drainLoadGenStream reads a response to its end and throws it away, returning
// the pooled buffers conn hands over and does not reclaim.
func drainLoadGenStream(ctx context.Context, s StreamRef) error {
	for {
		ev, err := s.Recv(ctx)
		if err != nil {
			return err
...
// runLoadGen drives one() b.N times across workers goroutines. runLoadGenN is
// the same for a one() that completes perCall requests at a time.
func runLoadGen(b *testing.B, wc *lgWriteCounter, workers int, one func() error) {
	b.Helper()
	runLoadGenN(b, wc, workers, 1, one)
}

func runLoadGenN(b *testing.B, wc *lgWriteCounter, workers, perCall int, one func() error) {
	b.Helper()
	if err := one(); err != nil { // warm the pools, the dynamic table and the stream pool
		b.Fatalf("warmup: %v", err)
	}
	wc.writes.Store(0)
	wc.bytes.Store(0)

	calls := int64((b.N + perCall - 1) / perCall)
	remaining := calls
	b.ReportAllocs()
	b.ResetTimer()
	var wg sync.WaitGroup
	for i := 0; i < workers; i++ {
		...
		go func() {
			...
			for atomic.AddInt64(&remaining, -1) >= 0 {
				if err := one(); err != nil {
					b.Errorf("request: %v", err) // Fatalf is illegal off the test goroutine
					return
				}
			}
		}()
	...
	n := float64(calls * int64(perCall))
	b.ReportMetric(float64(wc.writes.Load())/n, "writes/req")
	b.ReportMetric(float64(wc.bytes.Load())/n, "wirebytes/req")
}

// BenchmarkLoadGen_Fused_C* is issue #438 item 1: HEADERS+DATA+END_STREAM in one
// flush, which takes Unary's 2.002 writes/req to ~1.0 with no new API.
func BenchmarkLoadGen_Fused_C1(b *testing.B)  { benchLoadGenFused(b, 1, 64, 64) }
func BenchmarkLoadGen_Fused_C64(b *testing.B) { benchLoadGenFused(b, 64, 64, 64) }

// BenchmarkLoadGen_Batch* is issue #438 item 2: cross-stream coalescing, which
// takes writes/req below one — the regime GroupCommit demonstrated was reachable
// (0.125 on headers-only traffic) without GroupCommit's waiting.
func BenchmarkLoadGen_Batch8_C8(b *testing.B)   { benchLoadGenBatch(b, 8, 8, 64, 64) }
func BenchmarkLoadGen_Batch32_C8(b *testing.B)  { benchLoadGenBatch(b, 8, 32, 64, 64) }
func BenchmarkLoadGen_Batch32_C64(b *testing.B) { benchLoadGenBatch(b, 64, 32, 64, 64) }
