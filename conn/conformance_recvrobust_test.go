...
func TestConformance_RFC9113_Sec4_3_HeaderDecodeError_ConnCompressionError(t *testing.T) {
	...
	var ce *ConnError
	if !errors.As(err, &ce) {
		t.Fatalf("err = %v (%T), want *ConnError — a decode error is connection-scoped (§4.3), not a stream reset", err, err)
	}
	if ce.Code != frame.ErrCodeCompressionError {
		t.Errorf("code = %v, want COMPRESSION_ERROR (§4.3)", ce.Code)
	}
}
...
func TestConformance_RFC9113_Sec6_9_1_WindowUpdateOnClosedStream_Tolerated(t *testing.T) {
	...
	if err := c.onWindowUpdate(3, 100); err != nil {
		t.Errorf("WINDOW_UPDATE on closed stream 3: err = %v, want nil (§6.9.1 MUST NOT treat as error)", err)
	}
	...
	s.remoteEnded = true
	c.streams[1] = s
	if err := c.onWindowUpdate(1, 100); err != nil {
		t.Errorf("WINDOW_UPDATE on half-closed(remote) stream 1: err = %v, want nil (§6.9.1)", err)
	}
}
...
func TestConformance_RFC9113_Sec8_1_CompleteResponseNotDiscardedByTrailingRSTNoError(t *testing.T) {
	...
	if err := deliverBlock(t, h, 1, status("200"), true); err != nil {
		t.Fatalf("response HEADERS: %v", err)
	}
	ev := <-s.events
	if ev.Type != EventHeaders || !ev.EndStream || statusOf(ev.Headers) != "200" {
		t.Fatalf("response event = %+v, want a complete 200 response", ev)
	}

	// The server then asks us to abort the upload with RST_STREAM(NO_ERROR).
	if err := h.OnRSTStream(frame.FrameHeader{Type: frame.FrameRSTStream, StreamID: 1}, frame.ErrCodeNoError); err != nil {
		t.Fatalf("trailing RST(NO_ERROR): err = %v, want nil — must be tolerated (§8.1)", err)
	}
	// The reset is enqueued AFTER the complete response, never in place of it.
	ev2 := <-s.events
	if ev2.Type != EventReset || ev2.RSTCode != frame.ErrCodeNoError {
		t.Fatalf("post-response event = %+v, want EventReset(NO_ERROR) delivered after the intact response", ev2)
	}
}
