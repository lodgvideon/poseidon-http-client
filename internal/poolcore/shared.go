...
func DialAttempt[C any](env DialEnv, dial func(context.Context) (C, error)) (C, error) {
	...
	start := time.Now()
	env.Rec.DialAttempted()
	c, err := dial(ctx)
	dur := time.Since(start)

	env.Rec.ObserveDial(dur)
	if err != nil {
		env.Rec.DialFailed()
	}
	env.Obs.OnDial(DialEvent{Addr: env.Addr, Err: err, Duration: dur})
	return c, err
}
...
func DialObserved[C any](ctx context.Context, addr string, timeout time.Duration,
	rec pool.Recorder, obs pool.Observer, dial func(context.Context) (C, error),
) (C, error) {
	start := time.Now()
	rec.DialAttempted()
	...
	obs.OnDial(DialEvent{Addr: addr, Err: err, Duration: dur})
	return c, err
}
...
func NotifyConnClose(addr string, reason CloseReason, obs pool.Observer, rec pool.Recorder) {
	rec.ConnClosed()
	obs.OnConnClose(ConnCloseEvent{Addr: addr, Reason: reason})
}
