...
**Кодек без аллокаций.** Кодирование и декодирование фреймов и HPACK работают на 0 B/op, 0 allocs/op, и bench-гейт в CI роняет сборку при регрессии. При высоких частотах запросов в нагрузочном генераторе аллокации на каждый фрейм напрямую превращаются в давление на GC; этот кодек не добавляет ничего. Пакеты `frame`, `hpack` и `qpack` можно использовать отдельно.
