...
一个 Go 语言的底层 HTTP 客户端，从零实现了 HTTP/1.1、HTTP/2 和 HTTP/3 —— 自研的帧编解码、HPACK、QPACK，以及一套从零编写的 QUIC 协议栈。它不使用 `net/http`，也不依赖任何第三方协议库；直接依赖只有 `golang.org/x/net` 和 `golang.org/x/crypto`（用于 ChaCha20-Poly1305），TLS 1.3 来自标准库。三个协议版本共用同一套请求 API：`Do` 和 `DoStream`。它面向压测工具（load generator）以及需要精细控制连接、流和流量控制的场景 —— 不是 `net/http` 的通用替代品。
...
- **从零实现，几乎零依赖** —— 没有 `quic-go`，没有 `nghttp2`，没有 cgo；代码面小，便于审计。
- **零分配编解码器** —— 帧与 HPACK 编解码达到 0 B/op、0 allocs/op，由 CI 基准门禁强制保证。
- **精细控制** —— 流、流量控制窗口、SETTINGS、连接池策略、拥塞控制（NewReno 或 BBR）；这些都是 `net/http` 不暴露的旋钮。
- **内置压测所需的功能** —— 连接池、DNS 服务发现、重试、限流、钩子与指标，H2 和 H3 共用。
